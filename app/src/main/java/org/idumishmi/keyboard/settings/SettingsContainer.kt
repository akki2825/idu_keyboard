...
import org.idumishmi.keyboard.settings.screens.createAboutSettings
import org.idumishmi.keyboard.settings.screens.createAdvancedSettings
import org.idumishmi.keyboard.settings.screens.createAppearanceSettings
import org.idumishmi.keyboard.settings.screens.createGestureTypingSettings
import org.idumishmi.keyboard.settings.screens.createLayoutSettings
import org.idumishmi.keyboard.settings.screens.createPreferencesSettings
import org.idumishmi.keyboard.settings.screens.createToolbarSettings
...
class SettingsContainer(context: Context) {
    ...
    fun filter(searchTerm: String): List<Setting> {
        ...
        list.forEach { setting ->
            // Removed description check as it refers to `Setting` title, not a separate description string.
...
private fun createSettings(context: Context) = createAboutSettings(context) + createAppearanceSettings(context) +
        createPreferencesSettings(context) + createToolbarSettings(context) +
